...
const prepareUrl = (url: Nullable<string>) => {
  if (url === null || url === undefined) return '/'

  if (url.startsWith('/news')) return url.replace('/news', '')

  return url
}

const openUrlInNewTab = (url: Nullable<string>) => {
  if (url === null || url === undefined || url.startsWith('/news')) return false
...
export const Calendar: React.FC<Props> = () => {
  ...
  const events: CalendarEventType[] = nodes.map(({ title, event }) => ({
    ...
    link: {
      url: prepareUrl(event?.link?.url),
      newTab: openUrlInNewTab(event?.link?.url)
    }
  }))
